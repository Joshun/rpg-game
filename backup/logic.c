...
static int check_intersect(rpgTile *tile, gameData *data, int tolerance, int player_x1, int player_x4, int player_y1, int player_y4, int tile_x1, int tile_x4, int tile_y1, int tile_y4)
{
	player_x1 += tolerance;
	player_x4 -= tolerance;
	player_y1 += tolerance;
	...
	if( (player_x1 < tile_x4 ) && (player_x1 > tile_x1) && (player_x4 < tile_x4) && (player_x4 > tile_x1) && (player_y1 > tile_y1) && (player_y1 < tile_y4) && (player_y4 > tile_y1) && (player_y4 < tile_y4) )
	{
...
{	
	...
	for(i=0; i<max_y_iter; i++) {
		for(j=0; j<max_x_iter; j++) {
			...
			#endif
			check_intersect(&(tilearr[(max_x_iter * i) + j]), data, tolerance, player_x1, player_x4, player_y1, player_y4, cur_x, (cur_x + base_size), cur_y, (cur_y + base_size));
			#if DEBUG >= 2
